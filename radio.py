...
class RadioPlayer:
    def __init__(self):
        ...
        self.player_process = {"process": None}
        self.current_page = 1
        self.search_term = ""
    ...
    def play_station(self, station_idx: int):
        ...
        self.stop_station()
        try:
            stations = self.playlists.get(self.current_playlist, [])
            if 0 <= station_idx < len(stations):
                name, url = stations[station_idx]
                self.player_process["process"] = subprocess.Popen([
                    "ffplay", "-nodisp", "-hide_banner", "-loglevel", "panic", url
                ], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            print(f"Error playing station: {str(e)}")

    def stop_station(self):
        """Stop the currently playing station."""
        if self.player_process.get("process"):
            self.player_process["process"].terminate()
            self.player_process["process"] = None
            self.is_playing = False

    def resume_last_station(self):
        """Resume the last played station if available."""
    ...
    def monitor_playback(self):
        ...
        while True:
            if self.player_process["process"] and self.player_process["process"].poll() is not None:
                self.is_playing = False
    ...
    def display_menu(self) -> List[Tuple[str, str]]:
        """Display the main menu with current playlist information."""
        clear_screen()
        ...
        if self.is_playing and 0 <= self.current_station < len(filtered_stations):
            current_name, current_url = filtered_stations[self.current_station]
            print(f"{Fore.GREEN}Now Playing: {Fore.WHITE}{current_name}")
            print(f"{Fore.CYAN}{'=' * 50}")
        elif self.is_playing:
            print(f"\n{Fore.YELLOW}Playback status unclear - please refresh")
            print(f"{Fore.CYAN}{'=' * 50}")
        ...
        for i, (name, _) in enumerate(filtered_stations[start_idx:end_idx], start=start_idx + 1):
            station_idx = i - 1
            if station_idx == self.current_station and self.is_playing:
    ...
    def run(self):
        ...
        # Optionally resume last played station
        if self.last_played_station:
            choice = input("Resume last played station? (y/n): ").strip().lower()
            if choice == 'y':
            ...
            try:
                ...
                if choice == 'n':
                    if filtered_stations:
                        ...
                        play_station(filtered_stations[self.current_station][1], self.player_process)
                        self.is_playing = True
                elif choice == 'p':
                    if filtered_stations:
                        self.current_station = (self.current_station - 1) % len(filtered_stations)
                    ...
                    try:
                        station_number = int(input("Enter station number: ")) - 1
                        if 0 <= station_number < len(filtered_stations):
                            self.current_station = station_number
                            play_station(filtered_stations[self.current_station][1], self.player_process)
                            self.is_playing = True
                        else:
                            print(f"{Fore.RED}Invalid station number. Press Enter to continue.")
                            input()
                    except ValueError:
                        print(f"{Fore.RED}Invalid input. Press Enter to continue.")
                        input()
                elif choice == 'a':
                    ...
                    stop_station(self.player_process)
                    break
                else:
